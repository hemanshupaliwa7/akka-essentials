...
class AskSpec extends ScalaTestWithActorTestKit with AnyWordSpecLike {
  import AskSpec._

  "An authenticator" should {
    "fail to authenticate a non-registered user" in {
      import AuthManager._
      val authDb = testKit.spawn(KVActor())
      val authManager = testKit.spawn(AuthManager(authDb))
      val testProbe = createTestProbe[Any]()
      authManager ! RegisterUser("daniel", "rtjvm")
      authManager ! Authenticate("daniel", "rtjvm")
      testProbe.expectMessage(AuthSuccess)
    }
  }
...
object AskSpec {
  ...
  object AuthManager {
    ...
    def handleAuthentication(username: String, password: String,  authDb: ActorRef[KVActor.Command], actorRef: ActorRef[AuthManager.Command])
